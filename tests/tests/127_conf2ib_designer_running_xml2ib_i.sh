...
TEST_NAME="Conf XML -> load to server infobase (ibcmd) with designer running"
...
# Save existing RAS PIDs
pids_ras_before=$(pgrep -f "ras " 2>/dev/null || true)
...
"${V8_PATH}/1cv8" DESIGNER /IBConnectionString "Srvr=\"${V8_SRV_ADDR}\";Ref=\"${TMP_IB_NAME}\";" /DisableStartupDialogs &
DESIGNER_PID=$!
sleep 5
