...
TEST_NAME="Ext CFE -> load to server infobase (designer)"
TEST_OUT_PATH="${OUT_PATH}/$(basename "${BASH_SOURCE[0]}" .sh)"
TEST_OUT_PATH="${TEST_OUT_PATH// /_}"
TEST_CHECK_PATH="${TEST_OUT_PATH}/DataProcessors/Расш1_Обработка1.xml"
...
"${SCRIPTS_PATH}/conf2ib.sh" "${TEST_BINARY}/1cv8.cf" "/S${V8_SRV_ADDR}:${V8_SRV_REG_PORT}/${TMP_IB_NAME}"

"${SCRIPTS_PATH}/ext2ib.sh" "${TEST_BINARY}/Расширение1.cfe" "/S${V8_SRV_ADDR}:${V8_SRV_REG_PORT}/${TMP_IB_NAME}" "Расширение1"

"${SCRIPTS_PATH}/ext2xml.sh" "/S${V8_SRV_ADDR}:${V8_SRV_REG_PORT}/${TMP_IB_NAME}" "${TEST_OUT_PATH}" "Расширение1"
