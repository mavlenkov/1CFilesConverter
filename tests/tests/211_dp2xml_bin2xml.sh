...
TEST_NAME="DP (binary) -> XML"
TEST_OUT_PATH="${OUT_PATH}/$(basename "${BASH_SOURCE[0]}" .sh)"
TEST_OUT_PATH="${TEST_OUT_PATH// /_}"
TEST_CHECK_PATH="${TEST_OUT_PATH}/ВнешняяОбработка1.xml"

echo "==="
echo "Test ${TEST_COUNT}. ($(basename "${BASH_SOURCE[0]}" .sh)) ${TEST_NAME}"
echo "==="
"${SCRIPTS_PATH}/dp2xml.sh" "${FIXTURES_PATH}/bin/ВнешняяОбработка1.epf" "${TEST_OUT_PATH}"
