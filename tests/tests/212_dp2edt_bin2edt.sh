...
TEST_NAME="DP (binary) -> EDT"
TEST_OUT_PATH="${OUT_PATH}/$(basename "${BASH_SOURCE[0]}" .sh)"
TEST_OUT_PATH="${TEST_OUT_PATH// /_}"
TEST_CHECK_PATH="${TEST_OUT_PATH}/src/ExternalReports/ВнешнийОтчет1/"

echo "==="
echo "Test ${TEST_COUNT}. ($(basename "${BASH_SOURCE[0]}" .sh)) ${TEST_NAME}"
echo "==="
"${SCRIPTS_PATH}/dp2edt.sh" "${FIXTURES_PATH}/bin/ВнешнийОтчет1.erf" "${TEST_OUT_PATH}"
