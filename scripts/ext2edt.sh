...
if [[ "${V8_SRC_TYPE}" != "xml" ]]; then
    ...
    if [[ "${V8_CONVERT_TOOL}" == "designer" ]]; then
        ...
        if [[ -n "${V8_BASE_IB_SERVER:-}" ]]; then
            "${IBCMD_TOOL}" infobase config export --data="${IBCMD_DATA}" --dbms="${V8_DB_SRV_DBMS}" --db-server="${V8_DB_SRV_ADDR}" --db-name="${V8_BASE_IB_NAME}" --db-user="${V8_DB_SRV_USR:-}" --db-pwd="${V8_DB_SRV_PWD:-}" --user="${V8_IB_USER:-}" --password="${V8_IB_PWD:-}" --extension="${V8_EXT_NAME}" ${IBCMD_EXPORT_FLAGS} "${XML_PATH}"
        else
