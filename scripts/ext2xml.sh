...
init_common "Convert 1C configuration extension to 1C:Designer XML format"
...
if [[ "${SRC_EXT,,}" == ".cfe" ]]; then
    ...
    V8_SRC_TYPE="cfe"
elif [[ -d "${V8_SRC_PATH}/DT-INF" ]] && [[ -f "${V8_SRC_PATH}/src/Configuration/Configuration.mdo" ]]; then
...
# --- base_ib (for cfe and edt types) ---
if [[ "${V8_SRC_TYPE}" == "cfe" ]] || [[ "${V8_SRC_TYPE}" == "edt" ]]; then
    ...
    # --- load_cfe (only for CFE source, EDT goes directly to EDT export) ---
...
if [[ "${V8_CONVERT_TOOL}" == "designer" ]]; then
    run_designer "${V8_BASE_IB_CONNECTION}" /DumpConfigToFiles "${V8_DST_PATH}" -Extension "${V8_EXT_NAME}" -force
    print_designer_log "${V8_DESIGNER_LOG}"
else
