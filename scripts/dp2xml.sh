...
init_common "Convert 1C external data processors & reports to 1C:Designer XML format"
...
if [[ "${SRC_EXT,,}" == ".epf" ]]; then
    ...
    # Check for epf/erf files in directory
    local_found=0
...
if [[ "${V8_SRC_MASK}" == "DIR" ]]; then
    for f in "${V8_SRC_FOLDER}"/*.epf "${V8_SRC_FOLDER}"/*.erf; do
        [[ ! -f "${f}" ]] && continue
        local_name="$(basename "${f}" | sed 's/\.[^.]*$//')"
        echo "[INFO] Building ${local_name}..."
        run_designer "${V8_BASE_IB_CONNECTION}" /DumpExternalDataProcessorOrReportToFiles "${V8_DST_PATH}/${local_name}.xml" "${f}"
        print_designer_log "${V8_DESIGNER_LOG}"
    done
else
    ...
    echo "[INFO] Building ${local_name}..."
    run_designer "${V8_BASE_IB_CONNECTION}" /DumpExternalDataProcessorOrReportToFiles "${V8_DST_PATH}/${local_name}.xml" "${V8_SRC_FOLDER}/${local_file}"
    print_designer_log "${V8_DESIGNER_LOG}"
fi
ERROR_CODE=$?
