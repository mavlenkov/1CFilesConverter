...
* Скрипты `*.cmd` — для Windows
* Скрипты `*.sh` — для Linux (общая библиотека [common.sh](./scripts/common.sh))
...
* **V8_TEMP** - путь к каталогу для создания временных файлов в процессе конвертации (по умолчанию: Windows — `%TEMP%\1c`, Linux — `/tmp/1c`)
...
* **V8_TOOL** - путь к исполняемому файлу 1С:Предприятие (по умолчанию: Windows — `%PROGRAMW6432%\1cv8\%V8_VERSION%\bin\1cv8.exe`, Linux — `/opt/1cv8/x86_64/%V8_VERSION%/1cv8`)
* **IBCMD_TOOL** - путь к утилите управления автономным сервером `ibcmd` (по умолчанию: Windows — `%PROGRAMW6432%\1cv8\%V8_VERSION%\bin\ibcmd.exe`, Linux — `/opt/1cv8/x86_64/%V8_VERSION%/ibcmd`)
* **RING_TOOL** - путь к утилите ring из состава EDT (по умолчанию берется из переменной окружения `PATH`)
* **EDTCLI_TOOL** - путь к утилите 1cedtcli из состава EDT (по умолчанию: Windows — `%PROGRAMW6432%\1C\1CE\components\1c-edt-%V8_EDT_VERSION%*`, Linux — `/opt/1C/1CE/components/1c-edt-*`)
...
* **V8_UPDATE_DB** - если установлена в `1`, то после загрузки конфигурации или расширения в информационную базу (conf2ib, ext2ib) будет выполнено обновление конфигурации базы данных (только для Linux-скриптов)
* **V8_IB_USER** - имя пользователя информационной базы (используется при работе с конфигуратором)
...
### conf2ib ([cmd](./scripts/conf2ib.cmd) | [sh](./scripts/conf2ib.sh))
...
### conf2cf ([cmd](./scripts/conf2cf.cmd) | [sh](./scripts/conf2cf.sh))
...
### conf2xml ([cmd](./scripts/conf2xml.cmd) | [sh](./scripts/conf2xml.sh))
...
### conf2edt ([cmd](./scripts/conf2edt.cmd) | [sh](./scripts/conf2edt.sh))
...
### dp2epf ([cmd](./scripts/dp2epf.cmd) | [sh](./scripts/dp2epf.sh))
...
Для использования базовой конфигурации в различных форматах может потребоваться наличие скрипта conf2ib ([cmd](./scripts/conf2ib.cmd) | [sh](./scripts/conf2ib.sh))

### dp2xml ([cmd](./scripts/dp2xml.cmd) | [sh](./scripts/dp2xml.sh))
...
### dp2edt ([cmd](./scripts/dp2edt.cmd) | [sh](./scripts/dp2edt.sh))
...
### ext2ib ([cmd](./scripts/ext2ib.cmd) | [sh](./scripts/ext2ib.sh))
...
### ext2cfe ([cmd](./scripts/ext2cfe.cmd) | [sh](./scripts/ext2cfe.sh))
...
### ext2xml ([cmd](./scripts/ext2xml.cmd) | [sh](./scripts/ext2xml.sh))
...
### ext2edt ([cmd](./scripts/ext2edt.cmd) | [sh](./scripts/ext2edt.sh))
...
### edt-validate ([cmd](./scripts/edt-validate.cmd) | [sh](./scripts/edt-validate.sh))
